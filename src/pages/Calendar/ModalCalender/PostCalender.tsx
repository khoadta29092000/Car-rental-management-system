...
import { CarscheduleResult, postCarscheduleAsyncApi } from "../../../redux/CarscheduleReducer/CarscheduleReducer";
...
export const PostCalender = (props: any) => {
  ...
  const formik = useFormik<CarscheduleResult>({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      ...
      const actionAsyncLogin = postCarscheduleAsyncApi(values);
      const actionAsyncUpdate = putCarupdatestatusAsyncApi({ id, carStatusId });
      await Promise.all([dispatch(actionAsyncLogin), dispatch(actionAsyncUpdate)]);
      setSubmitting(false);
    }
  });
          ...
          <form onSubmit={formik.handleSubmit}>
            ...
            <DialogContent dividers>
              ...
              <div className=" mx-2 gap-2 gap-x-5">
                ...
                <div className="mt-2">
                <FormControl className="w-full mt-2">
  <InputLabel size="small">Biển số xe</InputLabel>
  <Select
    value={formik.values.carId}
    size="small"
    labelId="carId-id-label"
    id="car-1"
    name="carId"
